...
struct node*head,*tail=NULL,*cur,*t1,*t2,*temp;
...
void insert_at_pos()
{
	...
	cur->next = t1;
	t1->prev = cur;
   	printf("Given node %f is inserted at %d position\n",cur->data,pos);
}
...
{	
	float value;
	t1=head;
	printf("enter the position of deletion before:");
	scanf("%f",&value);
	t1=head;
...
{	
	float value;
	printf("enter the position of deletion after:");
	scanf("%f",&value);
	t1=head;
	while(t1!=NULL && t1->data!=value)
	{
